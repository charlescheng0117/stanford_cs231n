...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #row_max_scores = row_max_scores.reshape( (N, -1) ) # reshape to (N, 1) for broadcasting
    #mat_scores -= row_max_scores

    """
    correct_class_scores = mat_scores[np.arange(N), y] # shape: (N, )
    correct_class_scores = correct_class_scores.reshape( (N, -1) )
    ...
    """
    #exp_scores = np.exp(mat_scores)
    exp_scores = np.exp(scores)
    probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
    log_correct_probs = -np.log(probs[np.arange(N), y])
